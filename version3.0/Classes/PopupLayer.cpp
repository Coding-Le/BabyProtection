...
void PopupLayer::registerWithTouchDispatcher(){
	...
	//Director::sharedDirector()->getTouchDispatcher()->addTargetedDelegate(this, -128, true);
}
...
void PopupLayer::buttonCallBack(CCObject* pSender){
	...
	CCNode* node = dynamic_cast<cocos2d::Node*>(pSender);
...
void PopupLayer::onEnter(){
	...
	int i = 0;
    cocos2d:Vector<Node*>::iterator it = getMenuButton()->getChildren().begin();
	for (; it != getMenuButton()->getChildren().end();)
	{
		(*it)->setPosition(ccp(winSize.width / 2 - contentSize.width / 2 + btnWidth*(i + 1),winSize.height / 2 - contentSize.height / 3));
		i++;
	}
